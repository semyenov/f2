...
export interface PerformanceMetrics {
  ...
  memoryUsage: typeof process.memoryUsage extends () => infer T ? T : Record<string, number>
  cpuUsage: typeof process.cpuUsage extends () => infer T ? T : Record<string, number>
}
...
export class Profiler {
  ...
  constructor(private readonly config: ProfilerConfig = {}) {
    ...
    if (this.config.autoProfiling ?? false) {
  ...
  startSpan(
    name: string,
    metadata?: Record<string, unknown>
  ): PerformanceSpan & { end: (status?: 'success' | 'error' | 'cancelled') => void } {
  ...
  private startAutoProfiling(): void {
    ...
    setInterval(() => {
      ...
      if (this.config.thresholds) {
        if (
          this.config.thresholds.maxLatency != null &&
        ...
        if (
          this.config.thresholds.errorRate != null &&
          this.metrics.errorRate > this.config.thresholds.errorRate
        ) {
          console.warn(
            `⚠️ Error rate threshold exceeded: ${(this.metrics.errorRate * 100).toFixed(2)}%`
          )
        }

        if (this.config.thresholds.memoryUsage != null) {
...
export const ProfilerMiddleware = {
  ...
  express: (profiler: Profiler) => {
    return (req: IncomingMessage, res: ServerResponse) => {
      const span = profiler.startSpan(`${req.method} ${req.url}`, {
        method: req.method,
        url: req.url,
        headers: req.headers,
      })
      ...
      res.end = function (...args: Parameters<typeof originalEnd>) {
        ...
        originalEnd.apply(res, args as Parameters<typeof originalEnd>)
  ...
  graphql: (profiler: Profiler) => ({
    requestDidStart() {
      ...
      return {
        ...
        didResolveOperation(context: { operation: { name: string } }) {
          profiler.createChildSpan(requestSpan.id, `graphql.operation.${context.operation?.name}`)
        },

        executionDidStart() {
          return {
            willResolveField({ info }: { info: GraphQLResolveInfo }) {
              const fieldSpan = profiler.startSpan(
