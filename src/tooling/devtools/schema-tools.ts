...
import type { GraphQLSchema, GraphQLType, GraphQLNamedType, ConstDirectiveNode } from 'graphql'
import {
  isObjectType,
  isListType,
  isNonNullType,
...
export class SchemaVisualizer {
   ...
   */
  generateD3Data(): { nodes: unknown[]; links: unknown[] } {
    const nodes: unknown[] = []
    const links: unknown[] = []
  ...
  calculateComplexity(): SchemaComplexityMetrics {
    const typeCount = Array.from(this.types.values()).filter(
      t => !this.isBuiltinType(t.name)
    ).length
    ...
    this.types.forEach((type, name) => {
      if (isObjectType(type) && !this.isBuiltinType(name)) {
        ...
        fields.forEach(field => {
          ...
          if (this.isListField(fieldType?.type ?? new GraphQLScalarType({ name: 'Unknown' })))
            complexity += 2
          if (isNonNullType(fieldType?.type ?? new GraphQLScalarType({ name: 'Unknown' })))
            complexity += 1
        })
  ...
  private groupTypesByModule(): Map<string, string[]> {
    ...
    this.types.forEach((_type, name) => {
      if (!this.isBuiltinType(name)) {
        ...
        const module = name.includes('User')
          ? 'Users'
  ...
  private findLongestPath(from: string, visited: Set<string>): number {
    ...
    return 1 + Math.max(...outgoing.map(r => this.findLongestPath(r.to, new Set(visited))))
  }
}
