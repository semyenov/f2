...
export type SchemaLifecycleState =
  | Data.TaggedEnum<{
    ...
    readonly Validated: { readonly schema: DocumentNode; readonly entities: readonly ValidatedEntity[]; readonly version: string }
    readonly Composed: { readonly federatedSchema: GraphQLSchema; readonly subgraphs: readonly string[]; readonly version: string }
    readonly Deployed: { readonly federatedSchema: GraphQLSchema; readonly deploymentId: string; readonly version: string }
    readonly Deprecated: { readonly schema: DocumentNode; readonly replacedBy: string; readonly version: string }
  }>
...
export type SchemaEvolution =
  | Data.TaggedEnum<{
    readonly AddField: { readonly entityType: string; readonly fieldName: string; readonly fieldType: string; readonly isBreaking: boolean }
    readonly RemoveField: { readonly entityType: string; readonly fieldName: string; readonly isBreaking: boolean }
    readonly ChangeFieldType: { readonly entityType: string; readonly fieldName: string; readonly oldType: string; readonly newType: string; readonly isBreaking: boolean }
    readonly AddDirective: { readonly entityType: string; readonly fieldName: string | undefined; readonly directive: string; readonly isBreaking: boolean }
    readonly RemoveDirective: { readonly entityType: string; readonly fieldName: string | undefined; readonly directive: string; readonly isBreaking: boolean }
...
export const SchemaFirstService = Context.GenericTag<SchemaFirstService>("@federation/SchemaFirstService")
...
export const createSchemaFirstService = (): SchemaFirstService => ({
  parseSchemaDefinition: (schemaSource: string) =>
    pipe(
      ...
      Effect.mapError((error) => new SchemaFirstError({
  ...
  extractEntitiesFromSchema: (schema: DocumentNode) =>
    pipe(
      ...
      Effect.catchAll((error) =>
        Effect.fail(new SchemaFirstError({
  ...
  validateSchemaEvolution: (currentSchema: DocumentNode, proposedSchema: DocumentNode) =>
    pipe(
      ...
      Effect.catchAll((error) =>
        Effect.fail(new SchemaEvolutionError({
  ...
  generateResolverStubs: (entities: readonly ValidatedEntity[]) =>
    pipe(
      ...
      Effect.map((entities) => {
        const stubs = entities.map(entity => `
...
export const ${entity.typename}Resolvers = {
  ...
  ${entity.typename}: {
    ...
    ${key.field}: (parent: unknown) => parent.${key.field}`).join(',')}
      ...
      Effect.catchAll((error) =>
        Effect.fail(new CodeGenerationError({
  ...
  generateTypeDefinitions: (entities: readonly ValidatedEntity[], language: "typescript" | "go" | "java" | "python") =>
    pipe(
...
const generateTypeScriptTypes = (entities: readonly ValidatedEntity[]): Effect.Effect<string, never> =>
  Effect.succeed(
    entities.map(entity => `
    ...
    pipe(
      ...
      Match.tag("Validated", ({ schema: currentSchema }) =>
        pipe(
          ...
          Effect.mapError((error: SchemaFirstError) => new SchemaEvolutionError({
            ...
            pipe(
              ...
              Effect.flatMap((evolutions: readonly SchemaEvolution[]) => {
                ...
                if (hasBreakingChanges) {
                  return Effect.fail(new SchemaEvolutionError({
