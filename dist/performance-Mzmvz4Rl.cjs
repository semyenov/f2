...
const dataloader = require_errors.__toESM(require("dataloader"));
...
(function(_FederationErrorBoundaries) {
	...
	const createBoundedResolver = (subgraphId, resolver, config) => {
		...
		return (parent, args, context, info) => {
			...
			return (0, effect.pipe)(effect.Effect.tryPromise({
				...
				catch: (error) => require_errors.ErrorFactory.federation("Resolver execution failed", subgraphId, "execution", error)
			}), effect.Effect.timeout(timeout), effect.Effect.catchTag("TimeoutException", () => effect.Effect.fail(require_errors.ErrorFactory.timeout(`Subgraph ${subgraphId} timed out`, timeout.toString()))), circuitBreaker.protect, effect.Effect.tap((_result) => effect.Effect.sync(() => {
				...
				if (config.partialFailureHandling.allowPartialFailure === true) return effect.Effect.succeed(null);
				else return effect.Effect.fail(error);
	...
	*/
	const processPartialResults = (results, config) => {
		...
		if (config.allowPartialFailure === false) return effect.Effect.fail(require_errors.ErrorFactory.federation("Subgraph failures not allowed", void 0, "partial_failure", { failedSubgraphs: failed.map((f) => f.subgraphId) }));
		const criticalFailures = failed.filter((f) => Boolean(config.criticalSubgraphs?.includes(f.subgraphId) ?? false));
		if (criticalFailures.length > 0) return effect.Effect.fail(require_errors.ErrorFactory.federation("Critical subgraph failure", void 0, "critical_failure", { failedSubgraphs: criticalFailures.map((f) => f.subgraphId) }));
			...
			protect: (effect$1) => (0, effect.pipe)(effect.Effect.succeed(state), effect.Effect.flatMap((currentState) => {
				switch (currentState) {
					case "open": {
						...
						}), effect.Effect.flatMap(() => effect$1)) : effect.Effect.fail(require_errors.ErrorFactory.circuitBreaker(`Circuit breaker open for ${subgraphId}`, "open"));
					}
					...
					default: return (0, effect.pipe)(effect$1, effect.Effect.tap(() => effect.Effect.sync(() => {
	...
	*/
	const validateCircuitBreakerConfig = (config) => (0, effect.pipe)(effect.Effect.succeed(config), effect.Effect.filterOrFail((conf) => conf.failureThreshold > 0, () => require_errors.ErrorFactory.composition("Failure threshold must be greater than 0")), effect.Effect.filterOrFail((conf) => effect.Duration.toMillis(conf.resetTimeout) > 0, () => require_errors.ErrorFactory.composition("Reset timeout must be greater than 0")));
	/**
...
(function(_PerformanceOptimizations) {
	_PerformanceOptimizations.createOptimizedExecutor = (schema, config) => (0, effect.pipe)(effect.Effect.succeed(config), effect.Effect.flatMap((config$1) => validatePerformanceConfig(config$1).pipe(effect.Effect.mapError((error) => require_errors.ErrorFactory.composition(`Performance configuration invalid: ${error.message}`, schema.metadata.subgraphCount.toString(), "performance")))), effect.Effect.flatMap((validConfig) => effect.Effect.all({
		queryPlanCache: createQueryPlanCache(validConfig.queryPlanCache).pipe(effect.Effect.mapError((error) => require_errors.ErrorFactory.composition(`Query plan cache creation failed: ${error.message}`, void 0, "cache"))),
		dataLoader: createFederatedDataLoader(validConfig.dataLoaderConfig).pipe(effect.Effect.mapError((error) => require_errors.ErrorFactory.composition(`DataLoader creation failed: ${error.message}`, void 0, "dataloader"))),
		metricsCollector: createMetricsCollector(validConfig.metricsCollection).pipe(effect.Effect.mapError((error) => require_errors.ErrorFactory.composition(`Metrics collector creation failed: ${error.message}`, void 0, "metrics")))
	...
	*/
	const executeOptimizedQuery = (schema, query, variables, context, optimizations) => {
		...
		return (0, effect.pipe)(optimizations.queryPlanCache.get(queryHash), effect.Effect.flatMap((cachedPlan) => {
			...
			data: null,
			errors: [new graphql.GraphQLError(error.message || "Execution failed", void 0, void 0, void 0, void 0, error, {
	...
	*/
	const validatePerformanceConfig = (config) => (0, effect.pipe)(effect.Effect.succeed(config), effect.Effect.filterOrFail((conf) => conf.queryPlanCache.maxSize > 0, () => require_errors.ErrorFactory.validation("Query plan cache max size must be greater than 0", "maxSize")), effect.Effect.filterOrFail((conf) => conf.dataLoaderConfig.maxBatchSize > 0, () => require_errors.ErrorFactory.validation("DataLoader max batch size must be greater than 0", "maxBatchSize")));
...
Object.defineProperty(exports, 'createBasicOptimizedExecutor', {
  enumerable: true,
  get: function () {
    return createBasicOptimizedExecutor;
  }
});
Object.defineProperty(exports, 'createDevelopmentOptimizedExecutor', {
  enumerable: true,
  get: function () {
    return createDevelopmentOptimizedExecutor;
  }
});
Object.defineProperty(exports, 'createProductionBoundary', {
  enumerable: true,
  get: function () {
    return createProductionBoundary;
  }
});
Object.defineProperty(exports, 'createProductionOptimizedExecutor', {
  enumerable: true,
  get: function () {
    return createProductionOptimizedExecutor;
  }
});
Object.defineProperty(exports, 'createResilientBoundary', {
  enumerable: true,
  get: function () {
    return createResilientBoundary;
  }
});
Object.defineProperty(exports, 'createStrictBoundary', {
  enumerable: true,
  get: function () {
    return createStrictBoundary;
  }
});