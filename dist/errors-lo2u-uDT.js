...
*/
var BaseDomainError = class extends Data.Error {
...
(function(_ErrorMatching) {
	_ErrorMatching.toUserMessage = (errorEffect) => pipe(errorEffect, Effect.match({
		onFailure: (error) => Match.value(error).pipe(Match.tag("ValidationError", (err) => `Invalid ${err.field ?? "field"}: ${err.message}`), Match.tag("SchemaValidationError", (err) => `Data format error: ${err.violations.map((v) => v.message).join(", ")}`), Match.tag("EntityResolutionError", (err) => `Could not find ${err.entityType ?? "entity"}: ${err.message}`), Match.tag("FieldResolutionError", (err) => `Field resolution failed for ${err.fieldName ?? "field"}: ${err.message}`), Match.tag("FederationError", (err) => `Federation error: ${err.message}`), Match.tag("CircuitBreakerError", () => "Service temporarily unavailable, please try again"), Match.tag("TimeoutError", () => "Request timed out, please try again"), Match.tag("CompositionError", (err) => `Schema composition failed: ${err.message}`), Match.tag("TypeConversionError", (err) => `Type conversion failed: ${err.message}`), Match.tag("RegistrationError", (err) => `Service registration failed: ${err.message}`), Match.tag("DiscoveryError", (err) => `Service discovery failed: ${err.message}`), Match.tag("HealthCheckError", (err) => `Health check failed: ${err.message}`), Match.exhaustive),
		onSuccess: () => "Operation completed successfully"
	}));
	_ErrorMatching.isRetryable = (error) => Match.value(error).pipe(Match.tag("ValidationError", () => false), Match.tag("SchemaValidationError", () => false), Match.tag("EntityResolutionError", () => true), Match.tag("FieldResolutionError", () => true), Match.tag("FederationError", () => false), Match.tag("CircuitBreakerError", () => true), Match.tag("TimeoutError", () => true), Match.tag("CompositionError", () => false), Match.tag("TypeConversionError", () => false), Match.tag("RegistrationError", () => true), Match.tag("DiscoveryError", () => true), Match.tag("HealthCheckError", () => true), Match.exhaustive);
	_ErrorMatching.getSeverity = (error) => Match.value(error).pipe(Match.tag("ValidationError", () => "medium"), Match.tag("SchemaValidationError", () => "medium"), Match.tag("EntityResolutionError", () => "high"), Match.tag("FieldResolutionError", () => "medium"), Match.tag("FederationError", () => "high"), Match.tag("CircuitBreakerError", () => "high"), Match.tag("TimeoutError", () => "medium"), Match.tag("CompositionError", () => "high"), Match.tag("TypeConversionError", () => "medium"), Match.tag("RegistrationError", () => "high"), Match.tag("DiscoveryError", () => "high"), Match.tag("HealthCheckError", () => "medium"), Match.exhaustive);
...
//#endregion
export { BaseDomainError, CircuitBreakerError, CompositionError, DiscoveryError, EntityResolutionError, ErrorFactory, ErrorMatching, FederationError, FieldResolutionError, HealthCheckError, RegistrationError, SchemaValidationError, TimeoutError, TypeConversionError, ValidationError };
//# sourceMappingURL=errors-lo2u-uDT.js.map