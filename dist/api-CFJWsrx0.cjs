...
const effect_Duration = require_errors.__toESM(require("effect/Duration"));
...
var Federation = class Federation {
	...
	*/
	static async create(config) {
		const layer = effect_Layer.mergeAll(require_subgraph.FederationComposerLive, require_logger.FederationLoggerLive, require_logger.FederationConfigLive);
		const effect = effect_Effect.gen(function* () {
			...
			const fullConfig = {
				...
				services,
				errorBoundaries: {
					subgraphTimeouts: Object.fromEntries(services.map((s) => [s.id, effect_Duration.seconds(config.resilience?.timeoutSeconds ?? 30)])),
					circuitBreakerConfig: {
						failureThreshold: config.resilience?.maxFailures ?? 5,
						resetTimeout: effect_Duration.seconds(30)
					},
				...
				performance: {
					queryPlanCache: {
						...
						ttl: effect_Duration.minutes(30)
					},
					dataLoaderConfig: {
			...
			const composer = yield* require_subgraph.FederationComposer;
			const result = yield* composer.compose(fullConfig);
			return new Federation(result.schema, config);
		});
		const providedEffect = effect_Effect.provide(effect, layer);
		return effect_Effect.runPromise(providedEffect);
	}
	/**
	* Create a simplified entity builder
	*/
	static createEntity(config) {
		return (0, effect_Function.pipe)(effect_Effect.gen(function* () {
			let builder = new require_logger.FederationEntityBuilder(config.typename, config.schema, config.keys);
		...
		}), effect_Effect.map((validatedEntity) => require_logger.toFederationEntity(validatedEntity, config.resolveReference)));
	}
	/**
...
*/
var SchemaValidationError = class extends effect_Data.TaggedError("SchemaValidationError") {};
/**
...
const validateSchema = (builder) => (0, effect_Function.pipe)(effect_Effect.succeed(builder.schema), effect_Effect.flatMap((schema) => {
	if (schema === void 0) return effect_Effect.fail([new SchemaValidationError({
...
const handleValidationErrors = (errors) => (0, effect_Function.pipe)(effect_Match.value(errors), effect_Match.when((errs) => errs.length > 0 && errs[0] instanceof SchemaValidationError, (errs) => {
...
require_errors.__export(advanced_exports, {
...
var api_exports = {};
require_errors.__export(api_exports, {
...
Object.defineProperty(exports, 'QuickEntityBuilder', {
  enumerable: true,
  get: function () {
    return QuickEntityBuilder;
  }
});
Object.defineProperty(exports, 'advanced_exports', {
  enumerable: true,
  get: function () {
    return advanced_exports;
  }
});
